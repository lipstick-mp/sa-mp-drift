...
//HTTP'const std::string &text, long time, int style
/*int HTTP(int index,int type, std::string &url, std::string &data, std::string &callback) {
    static AMX_NATIVE native = Wrapper::GetInstance()->GetNative("HTTP");
    cstring url_(url.begin(), url.end());
    cstring data_(data.begin(), data.end());
    cstring callback_(callback.begin(), callback.end());

    cell params[] = {
        5 * 4,
		index,
		type,
        reinterpret_cast<cell>(url_.c_str()),
        reinterpret_cast<cell>(data_.c_str()),
        reinterpret_cast<cell>(callback_.c_str())
    };
    return native(&::fakeAmx, params);
}*/
} // namespace sampgdk
